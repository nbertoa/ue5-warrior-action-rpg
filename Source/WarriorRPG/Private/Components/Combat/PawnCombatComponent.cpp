...
{
	const FString OwnerName = GetOwner()
		                          ? GetOwner()->GetName()
		                          : TEXT("UnknownOwner");
	...
	if (!ensureMsgf(!CarriedWeaponByTag.Contains(InWeaponTag),
	                TEXT("[%s] UPawnCombatComponent::RegisterSpawnedWeapon — a weapon with tag "
		                "[%s] is already registered. Ignoring duplicate registration."),
	                *OwnerName,
	...
	if (!ensureMsgf(IsValid(InWeapon),
	                TEXT("[%s] UPawnCombatComponent::RegisterSpawnedWeapon — InWeapon is null "
		                "for tag [%s]. Verify the weapon was spawned successfully before registration."),
	                *OwnerName,
	                *InWeaponTag.ToString()))
	{
		return;
...
void UPawnCombatComponent::ToggleCurrentEquippedWeaponCollision(bool bShouldEnable)
{
	...
	BoxCollision->SetCollisionEnabled(bShouldEnable
