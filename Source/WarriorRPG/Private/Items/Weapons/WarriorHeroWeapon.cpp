	...
	if (!GrantedAbilitySpecHandles.IsEmpty())
	{
		...
		if (!OwningCharacter)
		{
			...
			UE_LOG(LogWarriorRPG,
			       Warning,
			       TEXT("%s: EndPlay — GrantedAbilitySpecHandles is not empty but owner is null "
				       "or not a AWarriorBaseCharacter. Ability specs may be orphaned."),
			       *GetName());
		...
		else
		{
			UE_LOG(LogWarriorRPG,
			       Warning,
			       TEXT("%s: EndPlay — Could not retrieve ASC from owning character [%s]. "
				       "Ability specs may be orphaned."),
			       *GetName(),
