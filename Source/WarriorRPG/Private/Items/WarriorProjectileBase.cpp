                                             ...
                                             const FHitResult& Hit)
{
	...
	if (!ensureMsgf(InstigatorPawn,
	                TEXT("AWarriorProjectileBase::OnProjectileHit — Projectile [%s] has no Instigator set. ") TEXT(
		                "Verify the spawner calls SetInstigator before activating the projectile."),
	                *GetActorNameOrLabel()))
	{
		Destroy();
		return;
