                                                              ...
                                                              int32 ApplyLevel)
{
	...
	if (!ensureMsgf(InASC,
	                TEXT("%s (UDataAsset_StartupDataBase): GiveToAbilitySystemComponent — InWarriorASC is null. "
		                "Verify the owning character created its ASC in the constructor."),
	                *GetName()))
	{
		return;
	}
                                                ...
                                                int32 ApplyLevel)
{
	for (const TSubclassOf<UWarriorGameplayAbility>& Ability : InAbilities)
	{
		...
		if (!ensureMsgf(Ability,
		                TEXT("%s (UDataAsset_StartupDataBase): Found a null ability entry in GrantAbilities. "
			                "Check the StartupData asset for empty slots in the ability arrays."),
		                *GetName()))
		{
			continue;
		}
