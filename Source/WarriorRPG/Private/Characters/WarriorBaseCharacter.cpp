...
void AWarriorBaseCharacter::PossessedBy(AController* NewController)
{
	...
	if (!ensureMsgf(WarriorAbilitySystemComponent,
	                TEXT("%s: WarriorAbilitySystemComponent is null in PossessedBy. "
		                "This should never happen — the component is created in the constructor."),
	                *GetName()))
	{
		return;
	}
	...
	ensureMsgf(!CharacterStartupData.IsNull(),
	           TEXT("%s: CharacterStartupData is not assigned. "
		           "Assign a StartupData asset in the Blueprint defaults to grant initial abilities."),
	           *GetName());
}
