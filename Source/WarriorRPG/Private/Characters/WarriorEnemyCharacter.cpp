...
void AWarriorEnemyCharacter::InitEnemyStartupData()
{
	if (!ensureMsgf(!CharacterStartupData.IsNull(),
	                TEXT("[AWarriorEnemyCharacter] CharacterStartupData is null on %s — "
		                "assign it in the Blueprint defaults."),
	                *GetName()))
	{
		return;
	}
...
void AWarriorEnemyCharacter::OnStartupDataLoaded()
{
	...
	else
	{
		...
		UE_LOG(LogWarriorRPG,
		       Warning,
		       TEXT("[AWarriorEnemyCharacter] Startup data asset failed to load on %s. "
			       "The enemy will have no abilities or attributes."),
		       *GetName());
	}
}
