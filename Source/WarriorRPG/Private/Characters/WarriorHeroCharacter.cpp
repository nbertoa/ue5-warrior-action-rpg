...
void AWarriorHeroCharacter::BeginPlay()
{
	...
	DebugHelper::Print(TEXT("AWarriorHeroCharacter::BeginPlay — Hero initialized successfully"));
}
...
void AWarriorHeroCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (!ensure(InputConfigDataAsset))
	{
		UE_LOG(LogWarriorRPG,
		       Error,
		       TEXT("%s: InputConfigDataAsset is null in SetupPlayerInputComponent. "
			       "Assign a valid DataAsset_InputConfig in the Blueprint defaults."),
		       *GetName());
		return;
	}
...
void AWarriorHeroCharacter::PossessedBy(AController* NewController)
{
	...
	if (!ensureMsgf(LoadedData,
	                TEXT("%s: Failed to load CharacterStartupData. "
		                "The asset path is set but the asset could not be loaded from disk."),
	                *GetName()))
	{
		return;
	}
...
void AWarriorHeroCharacter::Input_SwitchTargetCompleted(const FInputActionValue& InputActionValue)
{
	...
	const FGameplayTag EventTag = SwitchDirection.X > 0.0f
