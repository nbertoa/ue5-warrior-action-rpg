...
UWarriorAbilitySystemComponent* UWarriorFunctionLibrary::NativeGetWarriorASCFromActor(AActor* InActor)
{
	...
	if (!ensureMsgf(ASC,
	                TEXT("UWarriorFunctionLibrary::NativeGetWarriorASCFromActor — "
		                "Actor [%s] does not have an AbilitySystemComponent. "
...
UPawnCombatComponent* UWarriorFunctionLibrary::NativeGetPawnCombatComponentFromActor(AActor* InActor)
{
	...
	return PawnCombatInterface != nullptr
		       ? PawnCombatInterface->GetPawnCombatComponent()
		       : nullptr;
}
	...
	EWarriorValidType& OutValidType)
{
	...
	OutValidType = CombatComponent != nullptr
		               ? EWarriorValidType::Valid
...
TSubclassOf<UWarriorActivatableWidget> UWarriorFunctionLibrary::GetWidgetClassByTag(FGameplayTag InWidgetTag)
{
	...
	checkf(WarriorUISettings->WidgetClassByTag.Contains(InWidgetTag),
	       TEXT("UWarriorFunctionLibrary::GetWidgetClassByTag — " "No widget class registered for tag [%s]. "
		       "Add the entry to Warrior UI Settings in Project Settings."),
                                                                         ...
                                                                         const FGameplayEffectSpecHandle& InSpecHandle)
{
	...
	if (!ensureMsgf(SourceASC,
	                TEXT("UWarriorFunctionLibrary::ApplyGameplayEffectSpecHandleToTargetActor — ") TEXT(
		                "InInstigator [%s] does not resolve to a valid ASC."),
	...
	if (!ensureMsgf(TargetASC,
	                TEXT("UWarriorFunctionLibrary::ApplyGameplayEffectSpecHandleToTargetActor — ") TEXT(
		                "InTargetActor [%s] does not resolve to a valid ASC."),
