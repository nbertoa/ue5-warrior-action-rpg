	...
	if (OwningCharacter)
	{
		...
		if (!OwningHeroCharacter)
		{
			UE_LOG(LogWarriorRPG,
			       Warning,
			       TEXT("UWarriorHeroAnimInstance::NativeInitializeAnimation — "
				       "OwningCharacter is valid but is not an AWarriorHeroCharacter. "
				       "This AnimInstance should only be used on hero character Blueprints."));
		}
	}
}
