...
UAbilityTask_WaitSpawnEnemies* UAbilityTask_WaitSpawnEnemies::WaitSpawnEnemies(UGameplayAbility* OwningAbility,
                                                                               ...
                                                                               TSoftClassPtr<AWarriorEnemyCharacter>
...
void UAbilityTask_WaitSpawnEnemies::OnEnemyClassLoaded()
{
	...
	for (int32 i = 0; i < CachedNumToSpawn; i++)
	{
		...
		const FRotator SpawnFacingRotation = AbilitySystemComponent->GetAvatarActor()->GetActorForwardVector().
		...
		if (!ensureMsgf(IsValid(SpawnedEnemy),
		                TEXT("[UAbilityTask_WaitSpawnEnemies] SpawnActor returned null "
			                "for class [%s] at index %d. NavMesh location or collision may be invalid."),
		                *LoadedClass->GetName(),
		                i))
		{
			continue;
