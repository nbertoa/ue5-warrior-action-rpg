...
void UWarriorAbilitySystemComponent::GrantHeroWeaponAbilities(
	const TArray<FHeroGameplayAbilityInfo>& InDefaultWeaponAbilities,
	int32 ApplyLevel,
	TArray<FGameplayAbilitySpecHandle>& OutGameplayAbilitySpecHandles)
{
	if (!ensureMsgf(!InDefaultWeaponAbilities.IsEmpty(),
	                TEXT("UWarriorAbilitySystemComponent::GrantHeroWeaponAbilities — "
		                "InDefaultWeaponAbilities is empty. "
		                "Add ability entries to the weapon's DefaultWeaponGameplayAbilities array.")))
	{
		return;
	}
...
void UWarriorAbilitySystemComponent::RemoveGrantedHeroWeaponAbilities(
	TArray<FGameplayAbilitySpecHandle>& InOutSpecHandles)
{
	if (!ensureMsgf(!InOutSpecHandles.IsEmpty(),
	                TEXT("UWarriorAbilitySystemComponent::RemoveGrantedHeroWeaponAbilities — "
		                "InOutSpecHandles is empty. "
		                "Ensure GrantHeroWeaponAbilities was called when the weapon was equipped.")))
	{
		return;
	}
...
bool UWarriorAbilitySystemComponent::TryActivateAbilityByTag(FGameplayTag AbilityTagToActivate)
{
	...
	if (FoundAbilitySpecs.IsEmpty())
	{
		UE_LOG(LogWarriorRPG,
		       Warning,
		       TEXT("UWarriorAbilitySystemComponent::TryActivateAbilityByTag — "
			       "No activatable ability found for tag [%s] on [%s]. "
			       "Verify the ability has the tag set in its OwnedGameplayTags and is granted to this ASC."),
