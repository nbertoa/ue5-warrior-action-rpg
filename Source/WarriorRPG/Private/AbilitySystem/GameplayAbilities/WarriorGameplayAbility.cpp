                                            ...
                                            const FGameplayAbilitySpec& Spec)
{
	...
	if (!ensureMsgf(ActorInfo,
	                TEXT("%s (UWarriorGameplayAbility): OnGiveAbility — ActorInfo is null. "
		                "This should never happen as the engine provides it."),
	                *GetName()))
	{
		return;
	}
	...
	if (!ensureMsgf(ActorInfo->AbilitySystemComponent.IsValid(),
	                TEXT("%s (UWarriorGameplayAbility): OnGiveAbility — AbilitySystemComponent is invalid. "
		                "Cannot auto-activate ability."),
	                *GetName()))
	{
		return;
	}
                                         ...
                                         bool bWasCancelled)
{
	...
	if (!ensureMsgf(ActorInfo,
	                TEXT("%s (UWarriorGameplayAbility): EndAbility — ActorInfo is null. "
		                "This should never happen as the engine provides it."),
	                *GetName()))
	{
		return;
	...
	if (!ensureMsgf(ActorInfo->AbilitySystemComponent.IsValid(),
	                TEXT("%s (UWarriorGameplayAbility): EndAbility — AbilitySystemComponent is invalid. "
		                "Cannot clear ability."),
	                *GetName()))
	{
		return;
	}
...
UPawnCombatComponent* UWarriorGameplayAbility::GetPawnCombatComponentFromActorInfo() const
{
	...
	AActor* AvatarActor = CurrentActorInfo
		                      ? CurrentActorInfo->AvatarActor.Get()
		                      : nullptr;

	if (!ensureMsgf(AvatarActor,
	                TEXT("%s (UWarriorGameplayAbility): GetPawnCombatComponentFromActorInfo failed. "
		                "AvatarActor is null. This occurs if the ability is still active after the pawn is destroyed."),
	                *GetName()))
	{
		return nullptr;
	}
	...
	ensureMsgf(PawnCombatComponent,
	           TEXT("%s (UWarriorGameplayAbility): Avatar [%s] has no UPawnCombatComponent. "
		           "This ability was designed to run on combat-capable pawns."),
	           *GetName(),
	           *AvatarActor->GetName());
...
UWarriorAbilitySystemComponent* UWarriorGameplayAbility::GetWarriorAbilitySystemComponentFromActorInfo() const
{
	...
	UWarriorAbilitySystemComponent* ASC = Cast<UWarriorAbilitySystemComponent>(
	...
	EWarriorSuccessType& OutSuccessType)
{
	...
	OutSuccessType = ActiveGEHandle.WasSuccessfullyApplied()
		                 ? EWarriorSuccessType::Succeeded
