...
UCommonActivatableWidgetContainerBase* UWarriorPrimaryLayout::FindWidgetStackByTag(const FGameplayTag& InTag) const
{
	...
	checkf(WidgetStackByTag.Contains(InTag),
	       TEXT("UWarriorPrimaryLayout::FindWidgetStackByTag — " "No widget stack registered under tag [%s]. "
		       "Ensure RegisterWidgetStack is called for this layer during widget initialization."),
	       *InTag.ToString());
                                                ...
                                                UCommonActivatableWidgetContainerBase* InStack)
{
	...
	if (WidgetStackByTag.Contains(InStackTag))
	{
		...
		UE_LOG(LogWarriorRPG,
		       Warning,
		       TEXT("UWarriorPrimaryLayout::RegisterWidgetStack — "
			       "Stack [%s] is already registered. Ignoring duplicate registration."),
